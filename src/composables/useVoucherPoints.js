import { ref } from 'vue'
import { apDungVoucher, suDungDiemTichLuy } from '@/service/banhang/banHangService'
...
export function useVoucherPoints(hoaDonHienTai, capNhatHoaDon) {
  ...
  const handleApplyVoucher = async (idPhieuGiamGia, voucherCode = null) => {
    ...
    try {
      ...
      const payload = idPhieuGiamGia ? { idPhieuGiamGia } : { maVoucher: voucherCode }
  ...
  const handleUsePoints = async (points) => {
    ...
    try {
      ...
      showError(
        error.response?.data?.message || 'Không thể sử dụng điểm tích lũy. Vui lòng thử lại!',
      )
    } finally {
      isLoading.value = false
    }
