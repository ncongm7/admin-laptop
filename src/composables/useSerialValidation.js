...
import { xacThucSerial } from '@/service/banhang/banHangService'
...
export function useSerialValidation(hoaDon) {
  ...
  const xacThucMotSerial = async (serialNumber, product) => {
    ...
    const daTonTai = validatedSerials.value.some((s) => s.serialNumber === serialNumber.trim())
  ...
  const xoaSerial = (serialNumber) => {
    const index = validatedSerials.value.findIndex((s) => s.serialNumber === serialNumber)
