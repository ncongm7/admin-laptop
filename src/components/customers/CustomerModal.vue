...
const selectProvince = async (province) => {
  ...
  // Load districts - LƯU Ý: Backend hiện tại không có API riêng cho quận/huyện
  // Tạm thời: Lấy danh sách xã/phường và extract unique districtCode
  try {
    loadingDistricts.value = true
    const response = await DiaChiService.getWardsByProvince(province.code)
    const data = response.data?.data || response.data || []

    // Extract unique districts từ danh sách wards
    const districtMap = new Map()
    data.forEach((ward) => {
      if (ward.districtCode && !districtMap.has(ward.districtCode)) {
        districtMap.set(ward.districtCode, {
          id: ward.districtCode,
          code: ward.districtCode,
          name: `Quận/Huyện ${ward.districtCode}`, // Tạm thời, cần API riêng
          provinceCode: province.code,
        })
      }
    })
    districts.value = Array.from(districtMap.values())
  } catch (error) {
...
const selectDistrict = async (district) => {
  ...
  // Load wards - LƯU Ý: Backend hiện tại không có API riêng cho quận/huyện
  // Tạm thời: Lấy tất cả wards từ tỉnh và filter theo districtCode
  try {
    loadingWards.value = true
    const response = await DiaChiService.getWardsByProvince(form.value.address.provinceCode)
    const data = response.data?.data || response.data || []

    // Filter wards theo districtCode
    wards.value = data
      .filter((ward) => ward.districtCode === district.code)
      .map((item) => ({
        id: item.id || item.code,
        code: item.code || item.id,
        name: item.name,
