<template>
      ...
      <div class="modal-content">
        ...
        >
          <h5 class="modal-title mb-0">
            Chi tiết bảo hành #WB{{ warranty.id.toString().padStart(6, '0') }}
          </h5>
              ...
              >
                <li>
                  <a
                    class="dropdown-item"
                    href="#"
                    @click.prevent="handleActionClick('appointment')"
                  >
            ...
            <div v-show="activeTab === 'info'" class="tab-pane fade show active">
              <div class="row">
                    ...
                    <div class="card-body">
                      <div class="row">
                        <div class="col-md-6">
                          ...
                          <div class="d-flex align-items-center mb-3">
                            ...
                            <div>
                              <h5 class="mb-0">
                                {{ warranty.productName || warranty.tenSanPham }}
                              </h5>
                              <p class="text-muted mb-0">
                                {{ warranty.productSpecs || warranty.maSanPham }}
                              </p>
                            </div>
                          </div>
                          <div class="mb-3">
                            <label class="form-label">Serial/IMEI</label>
                            <p class="fw-bold">{{ warranty.imei || warranty.soSerial || 'N/A' }}</p>
                          </div>
                          <div class="mb-3">
                        ...
                        <div class="col-md-6">
                          ...
                          <div class="d-flex align-items-center mb-3">
                            ...
                            <div>
                              <h5 class="mb-0">
                                {{ warranty.customerName || warranty.hoTenKhachHang }}
                              </h5>
                              <p class="text-muted mb-0">
                                {{ warranty.customerPhone || warranty.soDienThoai }}
                              </p>
                            </div>
                          </div>
                          <div class="mb-3">
                      ...
                      <div v-if="warranty.hinhAnh && warranty.hinhAnh.length > 0" class="mt-3">
                        ...
                        <div class="d-flex flex-wrap gap-2">
                          <img
                            v-for="(img, idx) in warranty.hinhAnh"
                            :key="idx"
                            :src="img"
                            class="img-thumbnail"
                ...
                <div class="col-md-4">
                  <div class="card mb-4">
                    ...
                    <div class="card-body">
                      ...
                      <div class="mb-3">
                        ...
                        <p class="fw-bold">{{ warranty.maPhieuBaoHanh || shortId(warranty.id) }}</p>
                      </div>
                      <div class="mb-3">
                  ...
                  <div class="card mb-4">
                    ...
                    <div class="card-body">
                      ...
                      >
                        ...
                        ></span>
                        <i v-else class="bi bi-box-seam me-2"></i>
                        Tiếp nhận sản phẩm
                      </button>

                      <!-- Status 1 → 2: Bắt đầu sửa -->
                      <button
                        v-if="currentStatus === 1"
                        class="btn btn-primary w-100 mb-2"
                        @click="advanceStatus(2)"
                        :disabled="updatingStatus"
                      >
                        <span
                          v-if="updatingStatus"
                      ...
                      <!-- Status 2 → 3: Sẵn sàng giao -->
                      <button
                        v-if="currentStatus === 2"
                        class="btn btn-info w-100 mb-2"
                        @click="advanceStatus(3)"
                        :disabled="updatingStatus"
                      >
                        <span
                          v-if="updatingStatus"
                      ...
                      <!-- Completed state -->
                      <div v-if="currentStatus === 4" class="alert alert-success mb-0">
                        <i class="bi bi-check-circle me-2"></i>
                        Phiếu bảo hành đã hoàn thành
                      </div>

                      <!-- Cancelled state -->
                      <div v-if="currentStatus === 5" class="alert alert-danger mb-0">
                        <i class="bi bi-x-circle me-2"></i>
                        Phiếu bảo hành đã bị hủy
                      </div>

                      <!-- Cancel button (available for non-completed/non-cancelled) -->
                      <button
                        v-if="currentStatus !== 4 && currentStatus !== 5"
                        class="btn btn-outline-danger w-100"
                        @click="cancelWarranty"
                        :disabled="updatingStatus"
                      >
                        <i class="bi bi-x-circle me-2"></i>
                        Hủy phiếu bảo hành
                  ...
                  <div class="card">
                    <div class="card-header">
                      <h6 class="mb-0">Hành động khác</h6>
                    </div>
                    <div class="card-body">
                  ...
                  <div v-if="phieuHenList.length > 0" class="card mb-4">
                    ...
                    <div class="card-body">
                      <div
                        v-for="phieuHen in phieuHenList"
                        :key="phieuHen.id"
                      ...
                      >
                        <div class="d-flex justify-content-between align-items-start mb-2">
                          <div>
                            ...
                            <span
                              :class="[
                                'badge ms-2',
                                getPhieuHenStatusBadgeClass(phieuHen.trangThai),
                              ]"
                            >
                        ...
                        <div class="small text-muted">
                          <div>
                            <i class="bi bi-calendar3 me-1"></i>{{ formatDate(phieuHen.ngayHen) }}
                          </div>
                          <div>
                            <i class="bi bi-clock me-1"></i>{{ formatTime(phieuHen.gioHen) }}
                          </div>
            ...
            <div v-show="activeTab === 'history'" class="tab-pane fade">
              <div class="card">
                ...
                <div class="card-body">
                  <WarrantyTimeline
                    :warranty="warranty"
                    :lichSuBaoHanh="lichSuBaoHanh"
                    :phieuHenList="phieuHenList"
                    ...
                    <table class="table table-bordered">
                      ...
                      <tbody>
                        <tr v-for="(ls, idx) in lichSuBaoHanh" :key="idx">
                          ...
                          <td class="text-end">{{ formatCurrency(ls.chiPhiPhatSinh || 0) }}</td>
                          <td class="text-center">
                            <span :class="['badge', ls.daThanhToan ? 'bg-success' : 'bg-warning']">
          ...
          <form @submit.prevent="addRepair">
            <div class="row mb-3">
              <div class="col-md-6">
                ...
                <input
                  v-model="repairForm.ngaySuaChua"
                  type="date"
                  class="form-control"
                  required
            ...
            <div class="mb-3">
              <label class="form-label"
                >Phương thức sửa chữa <span class="text-danger">*</span></label
              >
            ...
            <div class="mb-3">
              ...
              <textarea
                v-model="repairForm.ghiChuNhanVien"
                class="form-control"
                rows="3"
              ></textarea>
            </div>
  ...
  <CreateAppointmentModal
    ...
    :warranty="warranty"
    @close="showCreateAppointmentModal = false"
...
const warrantyProgress = computed(() => {
  const totalDays = Math.ceil(
    (new Date(props.warranty.ngayKetThuc) - new Date(props.warranty.ngayBatDau)) /
      (1000 * 60 * 60 * 24),
  )
  const passedDays = Math.ceil(
    (new Date() - new Date(props.warranty.ngayBatDau)) / (1000 * 60 * 60 * 24),
  )
  const percentage = Math.min(100, Math.max(0, (passedDays / totalDays) * 100))
  return { width: `${percentage}%` }
})

// Current status of warranty
const currentStatus = computed(() => {
  return props.warranty?.trangThai ?? props.warranty?.status ?? 0
})
...
const handleModalSuccess = (data) => {
  ...
  fetchHistory()
  fetchWarrantyDetail() // Refresh info status
  emit('refresh')
}
...
const advanceStatus = async (nextStatus) => {
  ...
  const statusName = warrantyStatusText(nextStatus)

  if (!confirm(`Xác nhận chuyển sang "${statusName}"?`)) {
    return
  }

  updatingStatus.value = true
  try {
    await updateTrangThai(props.warranty.id, nextStatus)
    alert(`✅ Đã chuyển sang "${statusName}"!`)
    emit('refresh')
...
const cancelWarranty = async () => {
  if (
    !confirm(
      '⚠️ Bạn có chắc muốn HỦY phiếu bảo hành này?\n\nLưu ý: Hành động này không thể hoàn tác!',
    )
  ) {
    return
  }
  ...
  updatingStatus.value = true
  try {
    await updateTrangThai(props.warranty.id, 5) // Status 5 = Cancelled
    alert('✅ Đã hủy phiếu bảo hành!')
    emit('refresh')
    const updatedWarranty = await getPhieuBaoHanhById(props.warranty.id)
    emit('warranty-updated', updatedWarranty)
  } catch (error) {
    console.error('❌ [WarrantyDetail] Lỗi khi hủy phiếu:', error)
    const errorMsg = error.response?.data?.message || error.message || 'Lỗi không xác định'
    alert('❌ Lỗi: ' + errorMsg)
  } finally {
    updatingStatus.value = false
