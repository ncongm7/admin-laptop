<template>
    <div class="warranty-management">
                    ...
                    <table class="table table-hover mb-0">
                        ...
                        <tbody>
                            ...
                            <tr v-for="item in filteredWarranties" :key="item.id">
                                ...
                                <td>
                                    <div class="fw-semibold">{{ item.hoTenKhachHang || item.customerName || 'N/A' }}
                                    </div>
                                    <div class="text-muted small">{{ item.soDienThoai || item.customerPhone || '—' }}
                                    </div>
                                </td>
                                <td>
                                ...
                                <td class="text-end">
                                    <button class="btn btn-sm btn-primary" @click="openDetail(item)">Xem chi
                                        tiết</button>
                                </td>
                            </tr>
                        </tbody>
        ...
        <WarrantyDetail v-if="showDetail && selectedWarranty" :warranty="selectedWarranty" @close="closeDetail"
            @refresh="handleRefresh" @warranty-updated="handleWarrantyUpdated" />
    </div>
</template>
...
const filteredWarranties = computed(() => {
    ...
    return warranties.value
        .filter((item) => {
            if (statusFilter.value !== '' && Number(statusFilter.value) !== (item.trangThai ?? item.status)) {
                return false
            }
            if (!text) return true
            const haystack = [
