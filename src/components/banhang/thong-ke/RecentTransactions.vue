...
import { getHoaDons, getHoaDonDetail, xacNhanDonHang, capNhatTrangThai } from '@/service/hoadon/hoaDonService'
...
const handleChuyenTrangThai = async () => {
  ...
  try {
    const responseData = await capNhatTrangThai(selectedTransaction.value.id, 4) // HOAN_THANH = 4

    // Service trả về response.data, nên kiểm tra trực tiếp
    if (responseData) {
