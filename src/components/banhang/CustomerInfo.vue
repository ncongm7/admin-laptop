<template>
  <div class="customer-info card">
    <div class="card-header bg-success text-white">
      <h6 class="mb-0"><i class="bi bi-person-circle"></i> Thông tin khách hàng</h6>
    </div>
    ...
    <div class="card-body">
      ...
      <div class="search-customer mb-3">
        ...
        <!-- Không tìm thấy -->
        <div
          v-if="showSearchResults && searchResults.length === 0 && searchKeyword"
          ...
          <div class="d-flex justify-content-between align-items-start mb-2">
            <div class="flex-grow-1">
              <div class="d-flex align-items-center gap-2 mb-1">
                <h6 class="customer-name mb-0">{{ selectedCustomer.hoTen }}</h6>
                <span v-if="selectedCustomer.isNew" class="badge bg-success badge-new">
              ...
              <div v-if="selectedCustomer.email" class="customer-email text-muted small mb-1">
                <i class="bi bi-envelope-fill"></i> {{ selectedCustomer.email }}
              </div>
              <div v-if="selectedCustomer.maKhachHang" class="customer-code text-muted small">
                <i class="bi bi-tag-fill"></i> Mã: {{ selectedCustomer.maKhachHang }}
              </div>
            </div>
            <button
...
// Watch customer prop - tự động ẩn badge "Mới" sau 5 giây
watch(
  () => props.customer,
  (newVal) => {
    if (newVal) {
      // Nếu khách hàng có flag isNew, hiển thị badge và tự động ẩn sau 5 giây
      selectedCustomer.value = { ...newVal, isNew: newVal.isNew || false }
      if (newVal.isNew) {
        setTimeout(() => {
          if (selectedCustomer.value && selectedCustomer.value.isNew) {
            selectedCustomer.value.isNew = false
          }
        }, 5000) // Ẩn badge sau 5 giây
      }
    } else {
